...
"""
    ...
    if data.split("-", 1)[0] == "BATCH":
        sts = await message.reply("**sᴇɴᴅɪɴɢ ғɪʟᴇs....**")
    ...
    buttons = [[
        InlineKeyboardButton('➕ ᴀᴅᴅ ʙᴏᴛ ᴛᴏ ʏᴏᴜʀ ɢʀᴏᴜᴘ ➕', url="http://t.me/CL_FILTER_BOT?startgroup=true")
    ]]
    await client.send_cached_media(
        chat_id=message.from_user.id,
        file_id=file_id,
...
@Client.on_message(filters.command('channel_list') & filters.user(ADMINS))
...
@Client.on_message(filters.command('hello1') & filters.private)
